...
export function handlePushNotificationClose(sw: PlatformSWScope) {
	...
	sw.addEventListener('notificationclose', e => {
		e.waitUntil(() => {
			const { data: o } = e.notification;
			const { trackingInfo: i } = o;
			const s = JSON.parse(i);

			sw.logger.logEvent('PushNotificationDismissed', {
